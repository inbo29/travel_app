...
const QUICK_ACCESS_ITEMS = [
    ...
    { name: 'Tickets', icon: '🎟️', desc: 'Bus / Train / Show' },
    { name: 'Wallet', icon: '💳', desc: 'Exchange' },
    { name: 'Translate', icon: '🗣️', desc: 'Voice & Text' },
    { name: 'Market', icon: '📊', desc: 'Live Rates' },
    { name: 'Map', icon: '🗺️', desc: 'Safety Zones' },
    { name: 'Travel Log', icon: '📔', desc: 'My History' },
]
        ...
        <div className="space-y-20">
                ...
                <div className="lg:col-span-7 space-y-8">
                    ...
                    <div className="space-y-4">
                        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-[1.1]">
                            <span className="block opacity-90">Everything floats</span>
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-emerald-400">on nature.</span>
                        </h1>
                        <p className="text-lg md:text-xl text-slate-600 dark:text-white/60 max-w-xl leading-relaxed font-medium">
                            Experience the vast steppes and crystal clear lakes through premium, transparent calmness.
                        </p>
                    </div>
                        ...
                        {QUICK_ACCESS_ITEMS.map((item, idx) => (
                            <button
                                ...
                                className={`
                                    ...
                                    ${idx === 0 ? 'col-span-2 sm:col-span-1 lg:col-span-2 flex-row items-center !p-6' : ''}
                                `}
                            >
                                <div className="w-12 h-12 flex items-center justify-center text-3xl bg-black/[0.03] dark:bg-white/[0.05] rounded-2xl group-hover:scale-110 group-hover:bg-accent/10 transition-all duration-500">
                                    {item.icon}
                                </div>
                                <div className="text-left">
                                    <h4 className="font-bold text-slate-900 dark:text-white leading-tight">{item.name}</h4>
                                    <p className="text-[11px] font-medium text-slate-500 dark:text-white/40 mt-0.5">{item.desc}</p>
                                </div>
            ...
            <section className="">
                <div className="flex items-end justify-between mb-8">
                    <div>
                        <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Editor's Pick</h2>
                        <p className="text-slate-500 dark:text-white/50 font-medium mt-1">Hand-picked experiences for you</p>
                    </div>
                    <button className="text-accent font-bold text-sm hover:underline decoration-2 underline-offset-4">View All</button>
                </div>
                        ...
                        <div className="absolute bottom-10 left-8 right-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
                            <div className="space-y-3">
                                <h3 className="text-4xl md:text-5xl font-black text-white leading-tight">
                                    Hidden Valley <br /> Trek Experience
                                </h3>
                                <div className="flex items-center gap-4 text-white/70 font-semibold text-sm">
                                    <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-accent"></span> 3 Days</span>
                                    <span>•</span>
                                    <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-accent"></span> Professional Guide</span>
                                </div>
                            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <section>
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Recommended</h2>
                        <button className="text-xs font-bold text-slate-400 uppercase tracking-widest">See More</button>
                    </div>
                        ...
                        {[1, 2].map(i => (
                            <div key={i} className={`${glassClasses} group rounded-3xl p-4 flex items-center gap-5 hover:bg-white dark:hover:bg-white/10 cursor-pointer`}>
                                <div className="w-24 h-24 rounded-2xl overflow-hidden shrink-0">
                                    <img src={`https://images.unsplash.com/photo-1526481280693-3bfa75ac8efd?q=80&w=2670&auto=format&fit=crop&sig=${i}`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[10px] font-bold text-accent px-2 py-0.5 rounded-md bg-accent/10 border border-accent/20 uppercase">Cultural</span>
                                    </div>
                                    <h4 className="font-bold text-lg truncate text-slate-900 dark:text-white">Altai Mountains Expedition</h4>
                                    <p className="text-sm text-slate-500 dark:text-white/40 font-medium">Remote experience • 12 Days</p>
                                </div>
                                <div className="text-right px-2">
                                    <div className="text-accent font-black text-lg">$1,200</div>
                                    <div className="text-[10px] text-slate-400 font-bold uppercase mt-1">Per Person</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <section>
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Nearby You</h2>
                        <button className="text-xs font-bold text-slate-400 uppercase tracking-widest">See More</button>
                    </div>
                    <div className="grid grid-cols-1 gap-4">
                        {[1, 2].map(i => (
                            <div key={i} className={`${glassClasses} group rounded-3xl p-4 flex items-center gap-5 hover:bg-white dark:hover:bg-white/10 cursor-pointer`}>
                                <div className="w-24 h-24 rounded-2xl overflow-hidden shrink-0">
                                    <img src={`https://images.unsplash.com/photo-1528642463367-9d1a6994cd75?q=80&w=2670&auto=format&fit=crop&sig=${i}`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[10px] font-bold text-blue-500 px-2 py-0.5 rounded-md bg-blue-500/10 border border-blue-500/20 uppercase">Local Stay</span>
                                    </div>
                                    <h4 className="font-bold text-lg truncate text-slate-900 dark:text-white">Nomad Home Stay #{i}</h4>
                                    <p className="text-sm text-slate-500 dark:text-white/40 font-medium">2.5km away • Steppe View</p>
                                </div>
                                <div className="text-right px-2">
                                    <div className="text-accent font-black text-lg">$25</div>
                                    <div className="text-[10px] text-slate-400 font-bold uppercase mt-1">Per Night</div>
                                </div>
                            </div>
                        ))}
                    </div>
